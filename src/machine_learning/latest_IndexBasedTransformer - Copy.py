...
class latest_IndexBasedTransformer(TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        if self.max_events is None:
            # Calculate the maximum number of events per case
        ...
        for i in range(self.max_events):
            dt_index = grouped.nth(i)[[self.case_id_col] + self.dynamic_activity_col]
            dt_index.columns = [self.case_id_col] + ["%s,%s,"%(col, i+1) for col in self.dynamic_activity_col]
            dt_transformed = pd.merge(dt_transformed, dt_index, on=self.case_id_col, how="left")
      
        #static columns
        dt_index2=grouped.tail(1)[[self.case_id_col] + self.static_cat_cols+ self.static_num_cols + [col for col in self.cat_cols] + self.num_cols]
        dt_index2.columns = [self.case_id_col] + ["%s,"%col for col in self.static_cat_cols]+ ["%s"%col for col in self.static_num_cols]+ ["%s,last,"%col for col in self.cat_cols]+ ["%s,last"%col for col in self.num_cols]
        dt_transformed = pd.merge(dt_transformed, dt_index2, on=self.case_id_col, how="left")
        #print(dt_transformed)
        
        if y is None:
            y= grouped.tail(1)[self.label_col].apply(lambda x: True if x == self.pos_label else False)
    
        # one-hot-encode frequents cat cols and delete unfrequents ones
        # one-hot-encode cat cols
        
        if self.create_dummies:
            all_cat_cols = ["%s,last,"%col for col in self.cat_cols]+ ["%s,"%col for col in self.static_cat_cols]
        ...
        file_XT = os.path.join(settings.results_dir, "X_test.csv")
        dt_transformed.to_csv(file_XT, index=False) 
